...
:: Date/Time Modified:         06/22/2020  4:07am
...
for %%s in (/system /system/system) do (
	...
	>nul 2>&1 adb shell "cat %%s/build.prop" | findstr /r /c:"^persist.vendor.camera.HAL3.enabled=1"
	if %errorlevel% NEQ 0  >nul 2>&1 adb shell "echo persist.vendor.camera.HAL3.enabled^=1>> %%s/build.prop"
	>nul 2>&1 adb shell "setprop persist.vendor.camera.HAL3.enabled 1"
	>nul 2>&1 adb shell "cat %%s/build.prop" | findstr /r /c:"^persist.camera.HAL3.enabled=1"
	if %errorlevel% NEQ 0  >nul 2>&1 adb shell "echo persist.camera.HAL3.enabled^=1>> %%s/build.prop"
	>nul 2>&1 adb shell "setprop persist.camera.HAL3.enabled 1"
	>nul 2>&1 adb shell "cat %%s/build.prop" | findstr /r /c:"^persist.camera.eis.enable=1"
	if %errorlevel% NEQ 0  >nul 2>&1 adb shell "echo persist.camera.eis.enable^=1>> %%s/build.prop"
...
if [%recovery_fastboot%] == [1] (
	...
	if "%recoveryimg%"==""      set "recoveryimg=.\recovery\ofox.img"
